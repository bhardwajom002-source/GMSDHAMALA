...
def teacher_dashboard_ui():
    ...
    st.write("Manage students, mark attendance, export CSV, delete records.")
    df = get_all_students_df()
    if df.empty:
...
if mode=="home": show_home()
    ...
    if st.session_state.teacher_logged_in: teacher_dashboard_ui()
    else: teacher_login_ui()
# You can fill reset_password_ui, change_password_ui, student signup/login, assistant etc similarly
# Key fix: all `r[...]` replaced with `r.get(...)` and df columns normalized
